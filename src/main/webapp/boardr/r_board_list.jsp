...
List boardList = (List) request.getAttribute("r_boardlist");
List boardnList = (List) request.getAttribute("r_notice"); //공지사항 받아오는 것

int listcount = ((Integer) request.getAttribute("r_listcount")).intValue();
int nowpage = ((Integer) request.getAttribute("r_page")).intValue();
int maxpage = ((Integer) request.getAttribute("r_maxpage")).intValue();
int startpage = ((Integer) request.getAttribute("r_startpage")).intValue();
int endpage = ((Integer) request.getAttribute("r_endpage")).intValue();
%>
...
<body>
				...
				<div style="background-color: rgb(55, 55, 55); padding: 30px;">
					<div>
						<font size="6px" color="#fff"
							style="font-family: 'Gowun Dodum', sans-serif">방탈출 후기</font>
					</div>
					...
					<div class="textbox">
						...
						<table style="width: 100%">
							<tr align="center" valign="middle" bgcolor="#ffcc99">
								<td height="26" colspan="5">고객 후기 게시판</td>
							</tr>
							...
							for (int i = 0; i < boardList.size(); i++) {
								BoardrBean bl = (BoardrBean) boardList.get(i);
							%>
							...
							<tr align="center" valign="middle" bordercolor="#333333"
								...
								<td style="font-family: Tahoma; font-size: 10pt;">
									<div align=center>
										<a href="./BoardrDetailAction.re?r_num=<%=bl.getR_num()%>">
											<%=bl.getR_subject()%> <!-- 전달되는 것은 num이지만 보여주는 것은 subject -->
										</a>
									</div>
								</td>

								<td style="font-family: Tahoma; font-size: 10pt;">
									<div align="center"><%=bl.getR_id()%></div>
								</td>

								<td style="font-family: Tahoma; font-size: 10pt;">
									<div align="center"><%=bl.getR_date()%></div>
								</td>

								<td style="font-family: Tahoma; font-size: 10pt;">
									<div align="center"><%=bl.getR_readcount()%></div>
								</td>
							</tr>
 ...
 %> <a href="./BoardrList.re?r_page=<%=nowpage - 1%>">[이전]</a>&nbsp; <%
 }
 %> <%
 ...
 %> <a href="./BoardrList.re?r_page=<%=a%>">[<%=a%>]
								</a>&nbsp; <%
 }
 %> <%
 }
 ...
 %> <a href="./BoardrList.re?r_page=<%=nowpage + 1%>">[다음]</a> <%
 }
 %>
								</td>
							</tr>
							...
							<tr align="right">
								<td colspan="5"><a href="./BoardrWrite.re">[글쓰기]</a></td>
							</tr>
						</table>
					</div>
				</div>
